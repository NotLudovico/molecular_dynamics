...
class Random:
    ...
    def Gaussian(self, shape=None):
        if shape is None:
            self.idum, self.iy, self.switchGaussian, self.saveGaussian, temp = (
                _Gaussian(
        ...
        for j in range(point[i], point[i + 1]):
            ...
            if distance2 <= forcecutoff2:
                ...
                fmod = (
                    2.0
                    * 4.0
                    * (6.0 * invdistance6 * invdistance6 - 3.0 * invdistance6)
                    * invdistance2
                )
                fx = fmod * distancex
                fy = fmod * distancey
...
class SimpleMD:
    ...
    def randomize_velocities(self, temperature, masses, random):
        return np.sqrt(temperature / masses)[:, np.newaxis] * random.Gaussian(
            shape=(len(masses), 3)
        )
    ...
    def thermostat(self, masses, dt, friction, temperature, velocities, engint, random):
        ...
        velocities = c1 * velocities + c2[:, np.newaxis] * random.Gaussian(
    ...
    def run(self):
        ...
        engconf = _compute_forces(
            self.cell, self.positions, self.forcecutoff, nlist, point, forces
        )
        ...
        for istep in tqdm(range(self.nstep)):
            ...
            check_list = self.check_list(
                self.positions, positions0, self.listcutoff, self.forcecutoff
            )
            if check_list:
                _compute_list(self.cell, self.positions, self.listcutoff, nlist, point)
                positions0 = +self.positions
            ...
            if (istep + 1) % self.nstat == 0:
                ...
                self.write_statistics(
                    istep + 1, self.tstep, len(self.positions), engkin, engconf, engint
                )
